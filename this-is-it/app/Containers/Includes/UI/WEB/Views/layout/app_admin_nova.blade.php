...
<head>
    @include('includes::layout.template.head_admin')
...
<body>
    <div class="nova" id="nova">
        ...
        <div class="nova-dashboards">
            ...
            <div class="nova-sidebar">
                @include('includes::layout.template.sidebar_admin_nova')
            </div>
            <!-- End Menu -->
            <!-- Content -->
            <div class="contents">
    ...
    <!-- Change menu elem visual on URL
    <script>
        //Get URL
        var searchParams = window.location.href;
        //Get List of Links <a> (Link list <(") ) inside nova-menu
        var menu = document.getElementsByClassName("nova-menu")[0];
        var links_list = menu.getElementsByTagName("a");

        //Traverse through all links, find the target that has the same href
        for (var i = 0; i < links_list.length; i++) {
            //console.log(links_list[i].href);

            //Compare Href to URL
            if(links_list[i].href == searchParams) {
                //Param handler
                
                links_list[i].className = "dropdown-item"; //Set class

                links_list[i].style = "color: red; background-color: white;"; //Set style

                var span_text = links_list[i].getElementsByClassName("text")[0];
                span_text.innerHTML = '<i class="fa fa-circle fa-xs"></i>' + span_text.innerHTML;
