...
<body>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            var imagePreviewContainer = $('#gallery');
            var form = $('#your-form-id'); // Replace with the actual ID of your form element
            ...
            var imagesPreview = function(input, placeToInsertImagePreview) {
                if (input.files) {
                    ...
                    for (i = 0; i < filesAmount; i++) {
                        ...
                        var file = input.files[i]; // Store the current file in a variable

                        reader.onload = (function(file) { // Wrap the function in an IIFE
                            return function(event) {
                                ...
                                image.attr({
                                    ...
                                    'style': 'width: 16vw; border: 2px solid black; margin-bottom: 5px;',
                                    'alt': file.name // Use the file parameter here
                                });

                                image.on('click', function() {
                                    container.remove();

                                    // Add deleted image to the list
            ...
            $('#gallery-photo-add').on('change', function() {
                ...
                imagesPreview(this, placeToInsertImagePreview);
            });

            form.on('submit', function() {
                // Assign the value of the deletedImages array to the deleted[] input element
                var deletedInput = $('#deletedImages');

                deletedInput.val(deletedImages);
                        ...
                        <div class="card-body">
                            ...
                            <div class="input-group mb-3">
                                ...
                                <textarea class="form-control" aria-label="With textarea" id="description" name="description" rows="3"
                                    placeholder="Text [5:255]" required minlength="5" maxlength="255">{{ old('description') }}</textarea>
                            </div>
                            {{-- Form Group (Brand) --}}
                            @php
                                    ...
                                    <div id="role_table" class="btn-group" role="group"
                                        ...
                                        @foreach ($brands ?? [] as $brand)
                                            <input type="radio" class="btn-check" name="brand"
                                                id="brand_{{ $loop->iteration }}" value="{{ $brand }}" required>
